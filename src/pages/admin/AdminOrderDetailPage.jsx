...
const AdminOrderDetailPage = () => {
  ...
  const [isLoadingPrintLabel, setIsLoadingPrintLabel] = useState(false);
  const handlePrintLabel = async () => {
    try {
      setIsLoadingPrintLabel(true);
      const orderCode = [
        order?.ghn_order_code
      ];
      const response = await OrderService.getTokenPrintLabel(orderCode);
      if (response?.token) {
        const printUrl = `https://dev-online-gateway.ghn.vn/a5/public-api/printA5?token=${response.token}`;
        window.open(printUrl, '_blank');
      } else {
        notification.error({
          title: 'Lỗi',
          description: 'Không nhận được token in nhãn vận chuyển. Vui lòng thử lại.',
        });
      }
    } catch (error) {
      ...
      notification.error({
        title: 'Lỗi',
        description: 'Không thể tạo nhãn vận chuyển. Vui lòng thử lại.',
      });
    }
    finally {
      setIsLoadingPrintLabel(false);
    }
  }
        ...
        <PageHeader
          ...
          actions={<>
            ...
            <button
              ...
              className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-primary px-md py-2 text-on-primary transition-all hover:opacity-90 md:flex-none">
              {
                isLoadingPrintLabel ? (
                ...
                <section className="bg-surface-container-lowest border border-outline-variant rounded-xl p-md">
                  ...
                  <div className="bg-surface-container-low rounded-lg p-sm flex items-center justify-between gap-xs">
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs font-semibold ${
                      String(order.payment?.status ?? order.payment_status ?? '').toUpperCase() === 'PAID'
                        ? 'bg-emerald-100 text-emerald-700'
                        : String(order.payment?.status ?? order.payment_status ?? '').toUpperCase() === 'REFUNDED'
                          ? 'bg-violet-100 text-violet-700'
                          : 'bg-amber-100 text-amber-700'
                    }`}>
                      {getPaymentStatusLabel(order.payment?.status ?? order.payment_status)}
                    </span>
                  </div>
                </section>
              </div>
